...
Всегда читай все md файлы когда ты выполняешь задачу. а также /Users/zidansaparbegov/PycharmProjects/GroceryStore-front/openapi.json где у нас весь бэкенд расписан по каждой ручке 
...
Скрин дизайна которые скидываю я не 100 процент должны совпдать с тем что ты сделаешь, ты берешь и пишешь код уже по существу, смотря на json бэкенда и подходит ли оно нам 
Все фотки должны быть в .gitignore
Делать дизайн стильно и строго по тому, что у нас бэкенд выдает
Playwright не запускать
Фронт на всех устройствах должен смотреть корректно, на маленьких экранах на средних на больших
Недобавлять моковые данные, только из бэка получаем 
Всегда запускай на порту 3000  а если занят то просто пользуйся уже готовым 
---
